...
def camera_transformation_from_pose(azimutal, elevation):
    ...
    azimutal, elevation = (
        azimutal * 2.0 * np.pi / 360.0,
        elevation * 2.0 * np.pi / 360.0,
    )
    azimutal *= -1.0
    elevation *= -1.0
    r_y = np.array(
    ...
    r_z = np.array(
        [
            [np.cos(azimutal), -np.sin(azimutal), 0],
...
def generate_scene_struct(c2w, num_object=3, metadata=None):
    ...
    cam_behind = c2w.dot(np.array([-1.0, 0, 0]))
    cam_left = c2w.dot(np.array([0, 1.0, 0]))
    cam_up = c2w.dot(np.array([0, 0, 1.0]))
...
def randomly_perturb_objects(
    scene_struct, old_objects, max_retries=10, min_margin=0.01, min_dist=0.1
):
